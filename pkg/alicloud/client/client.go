...
// DetachECSInstancesFromSshKey finds all ECS instances and detach them from the specified SSH key pair.
func (c *ecsClient) DetachECSInstancesFromSshKey(keyName string) error {
	pageSize := 50
	...
	for _, ids := range datachInstanceBatches {
		...
		if detachResponse.FailCount != "0" {
			return fmt.Errorf("fail to detach keypair[%s] from instances: %v", keyName, detachResponse.Results)
		}
	}
